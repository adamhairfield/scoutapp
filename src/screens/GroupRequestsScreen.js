...
const GroupRequestsScreen = ({ navigation, route }) => {
  const { user } = useAuth();
  ...
  const loadRequests = async () => {
    try {
      setLoading(true);
      const groupRequests = await groupService.getGroupJoinRequests(groupId);
  ...
  const processRequest = async (requestId, status) => {
    try {
      const result = await groupService.processJoinRequest(requestId, status, user.id);
  ...
  const confirmProcessRequest = (request, status) => {
    ...
    const message = request.request_type === 'player'
      ? `${action} ${request.profiles.name}'s request to join the group?`
      : `${action} ${request.profiles.name}'s request for ${playerName} to join the group?`;
